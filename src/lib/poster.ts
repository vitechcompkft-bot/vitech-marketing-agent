...
/**
 * LIFESTYLE plakát render (TISZTA: jelenet-háttér + focím + alcím + logó + CTA) → PNG.
 * SAJÁT, kvóta nélküli renderelo (next/og), lásd ogPoster.ts. Nincs külso szolgáltatás/keret.
 */
export async function renderLifestylePosterPng(o: { bgUrl: string; headline: string; sub?: string }): Promise<string | null> {
  ...
  try {
    const { renderLifestylePosterOG } = await import("./ogPoster");
    const url = await renderLifestylePosterOG(o);
    if (url) return url;
    _lastLifestyleRenderError = "og render null (betu/feltöltés?)";
    return null;
  } catch (e: any) {
    _lastLifestyleRenderError = `og kivétel: ${e?.message || e}`;
    return null;
  }
}
